...
(function(factory, window) {
    ...
    var PopupLayoutLayer = L.Evented.extend({
        ...
        polylineList: [],
        _popupLayer: null,
        _area: (window.innerWidth * window.innerHeight) / 10, // 线长度相关
        ...
        addMarker: function(marker, toolTipContentHTML) {
            const self = this
            this._map.addLayer(marker);
            if (!this.options.showAll) {
                // this._area = (window.innerWidth * window.innerHeight) / 50
                this._area = 50000 // 线长度相关
                marker.options.toolTipContentHTML = toolTipContentHTML
                ...
                console.log("name",toolTipContentHTML);
                this._area = (window.innerWidth * window.innerHeight) / 40 // 线长度相关
                marker.bindTooltip(toolTipContentHTML, this.options.tooltipOptions);
        ...
        mouseClick: function(evt) {
            ...
            evt.originalEvent.path.forEach(element => {
                if (element.className && element.className.indexOf('leaflet-tooltip') > 0) {
                    isTooltipClick = true
                }
            })
            if (isTooltipClick || this.markerList.length === 0) {
                this._map.closePopup() //移除 popup
                this.tooltipClear()
                this.fire('click', evt.sourceTarget);
            }
            ...
            if (!evt.target.getTooltip()) {
                ...
                this.allMarkerList.forEach(makerItem => {
                    if (makerItem.getLatLng().lat === evt.target.getLatLng().lat && makerItem.getLatLng().lng === evt.target.getLatLng().lng) {
                        makerItem.bindTooltip(makerItem.options.toolTipContentHTML, this.options.tooltipOptions);
